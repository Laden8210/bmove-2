...
// check name if it is already in use
...
$stmt = $conn->prepare("INSERT INTO vehicles (vehicleid, name, platenumber, totalcapacitykg, status, baseprice, rateperkm, type, model, year, driver_uid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
$stmt->bind_param("sssissssssi",$uid, $name, $platenumber, $totalcapacitykg, $status, $baseprice, $rateperkm, $type, $model, $year, $driver_uid);
if ($stmt->execute()) {
    echo json_encode(['status' => 'success', 'message' => 'Truck created successfully', 'http_code' => 200]);
} else {
    echo json_encode(['status' => 'error', 'message' => 'Failed to create truck', 'http_code' => 500]);
}
$stmt->close();
$conn->close();
