...
// user name,phone number, full name, email, password, confirm password, otp code
...
if (!isset($request_body['otp_code'])) {
    echo json_encode(['status' => 'error', 'message' => 'OTP code is required', 'http_code' => 400]);
    exit;
}

$otp_code = filter_var($request_body['otp_code'], FILTER_SANITIZE_STRING);


if (!preg_match('/^\d{6}$/', $otp_code)) {
    echo json_encode(['status' => 'error', 'message' => 'Invalid OTP code format', 'http_code' => 400]);
    exit;
}

// CHECK IF OTP CODE IS SENT

if (!isset($_SESSION['otp'])) {
    echo json_encode(['status' => 'error', 'message' => 'Please request an OTP code first', 'http_code' => 400]);
    exit;
}

// CHECK TIME IF OTP IS EXPIRED

if (isset($_SESSION['otp_expiry']) && time() > $_SESSION['otp_expiry']) {
    echo json_encode(['status' => 'error', 'message' => 'OTP code has expired', 'http_code' => 400]);
    exit;
}

// CHECK IF OTP IS VALID
if (!$otpGenerator->validateOTP($otp_code)) {
    echo json_encode(['status' => 'error', 'message' => 'Invalid OTP code', 'http_code' => 400]);
    exit;
}
