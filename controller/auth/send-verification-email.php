...
if (!isset($_SESSION['verification_requests'])) {
    $_SESSION['verification_requests'] = 0;
}

if ($_SESSION['verification_requests'] >= 5) {
    echo json_encode(['status' => 'error', 'message' => 'Verification request limit exceeded. Please try again later.', 'http_code' => 429]);
    exit;
}
...
// Store token and email in session for use during registration
...
$result = $mailer->sendVerificationEmail($email, $verificationUrl);

if ($result['success']) {
    echo json_encode(['status' => 'success', 'message' => 'Verification link sent successfully', 'http_code' => 200]);
} else {
